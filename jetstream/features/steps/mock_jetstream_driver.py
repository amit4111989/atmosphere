...
from jetstream.tas_api import tacc_api_post, tacc_api_get
...
class TASAPIDriver(object):
    ...
    def report_project_allocation(self, report_id, username, project_name, su_total, start_date, end_date, queue_name,
                                  scheduler_id):
        """
        Send back a report
        """
        ...
        try:
            ...
            exc_message = ("Report %s produced an Invalid Response - Expected 'status' in the json response: %s" % (
            report_id, resp.text,))
            logger.exception(exc_message)
            raise ValueError(exc_message)

        if resp_status != 'success' or resp.status_code != 200:
            exc_message = ("Report %s produced an Invalid Response - Expected 200 and 'success' response: %s - %s" % (
    ...
    def get_user_allocations(self, username, include_expired=False, raise_exception=True):
        ...
        try:
            ...
            for project in projects:
                api_allocations = project['allocations'] if include_expired else select_valid_allocations(
                    project['allocations'])
...
def fill_user_allocation_sources(user_list,new_allocation):
    from core.models import AtmosphereUser
    driver = TASAPIDriver()
    allocation_resources = {}
    all_us = AtmosphereUser.objects.order_by('username')
    for user in AtmosphereUser.objects.order_by('username'):
        if user.username not in user_list:
            continue
        try:
            resources = fill_user_allocation_source_for(driver, user, new_allocation)
        except Exception as exc:
...
def fill_user_allocation_source_for(driver, user, new_allocation, force_update=True):
    allocation_list = find_user_allocation_source_for(driver, user)

    #Add new allocation to the list
    allocation_list.append(new_allocation)
